...
| `TrafficMonitorClaudeQuota.dll` | `CL 5h:`、`CL 7d:`，以及 Enterprise 账户的 `CL 1mo:` Claude 额度窗口 | 在插件选项中粘贴 claude.ai `sessionKey`，或先登录 Claude Code。 |
...
Claude 额度插件的网页会话查询结果与 `https://claude.ai/settings/usage` 一致，包括 Enterprise 月度 spend limit。打开 Claude 插件选项，从浏览器开发者工具的请求中复制 `sessionKey` cookie 值或完整 `Cookie` request header 并粘贴。插件只提取 `sessionKey`，并将其作为 TrafficMonitor 专用本地凭据保存到 Windows 凭据管理器。由于 Chrome 和 Edge 的 App-Bound Encryption 无法让第三方稳定自动导入 cookie，本插件不会读取浏览器 cookie 数据库。也可以先运行 `claude login`；未配置网页会话时，插件会读取 `~/.claude/.credentials.json` 并调用 Claude Code OAuth usage endpoint。还可以使用优先级更高的 `CLAUDE_AI_SESSION_KEY` 或 `CLAUDE_WEB_SESSION_KEY` 环境变量。
